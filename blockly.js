...
Blockly.Generator.generateGame = function (workspace, generator, code, ind) {
  code.push(Code.indent(ind) + '// generated by code4kids');
  code.push(Code.indent(ind) + 'var game = new Phaser.Game(' + Blockly.Block.GAME_WIDTH + ', ' + Blockly.Block.GAME_HEIGHT + ', Phaser.AUTO, \'content_game\', { preload: preload, create: create, update: update, render: render});');
  code.push('');
...
Blockly.Generator.generatePreload = function (workspace, generator, code, ind) {
  ...
  for (var i = 0; i < blocks.length; i++) {
    var block = blocks[i];
    if (block.getImage) {
      code.push(Code.indent(ind + 1) + 'game.load.image(\'' + block.getImage().key + '\', \'' + block.getImage().url + '\');');
    }
  }
